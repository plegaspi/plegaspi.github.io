...
ct.inputs.addAction('jump', [{"code":"keyboard.Space"},{"code":"keyboard.ArrowUp"}]);
ct.inputs.addAction('pause', [{"code":"keyboard.KeyP"}]);
...
ct.rooms.templates['main'] = {
    ...
    objects: JSON.parse('[{"x":-9,"y":543,"exts":{},"type":"ground-1"}]'),
    bgs: JSON.parse('[{"depth":-4,"texture":"background-1","extends":{"repeat":"no-repeat","shiftY":-608,"scaleX":0.86,"scaleY":1.47,"shiftX":0}}]'),
    tiles: JSON.parse('[{"depth":-10,"tiles":[],"extends":{}}]'),
    backgroundColor: '#000000',
    
    onStep() {
        if (this.gameStart && !this.gameEnd) {
    if (this.iterations = 0) this.mage.tex = "Mage_Run";
    this.gameStartTimer += 1;
    if (this.gameStartTimer >= 200) this.gameStartBreak = false;
    if (this.gameStartTimer%20 === 0 && !this.gameEnd) this.score += Math.floor(ct.delta);
    ...
    if (!this.gameStartBreak) {
        if (!this.obstacleIncoming) {
            this.obstacleIncoming = true;
            this.obstacleScale = ct.random.dice(0.75, 1, 1.5);
            let obstacleY = 0;
            if (this.obstacleScale === 0.75) {
                obstacleY = ct.camera.height - 120;
            } else if (this.obstacleScale === 1) {
                obstacleY = ct.camera.height - 130;
            } else {
                obstacleY = ct.camera.height - 175;
            }
            ct.types.copy('c1', ct.camera.width + 200, obstacleY);
        }
    }
} else {
    if (ct.actions.jump.pressed) {
        if (this.iterations != 0) {
            this.mage = ct.types.copy('mage', 53, 508);
        }
        this.mage.tex = "Mage_Run";
        this.gameOverLabel.visible = false;
        this.score = 0;
        this.lives = 3;
        this.obstacleTimer = 180;
        this.gameStartBreak = true;
        this.gameStartTimer = 0;
        this.gameEnd = false;
        this.obstacleIncoming = false;
        this.gameStart = true;
    }
}
...
if (this.gameEnd) {
    if (this.lives === 0) {
        ...
        localStorage.highScore = String(this.highScore);
...
this.highScore = ('highScore' in localStorage) ? Number(localStorage.highScore) :0;
this.highScoreLabel = new PIXI.Text('High Score: ' + this.highScore);
this.addChild(this.highScoreLabel);
this.highScoreLabel.x = 30;
...
this.gameOverLabel = new PIXI.Text('Game Over', ct.styles.get('ScoreText'));
this.addChild(this.gameOverLabel);  
this.gameOverLabel.x = ct.camera.width/2 - 50;
this.gameOverLabel.y = ct.camera.height/2 - 50;
this.gameOverLabel.depth = 1001;
this.gameOverLabel.visible = false;
this.mage = ct.types.copy('mage', 53, 508);
    },
    extends: {
...
ct.rooms.templates['starting'] = {
    name: 'starting',
    width: 800,
    height: 600,
    /* JSON.parse allows for a much faster loading of big objects */
    objects: JSON.parse('[{"x":-9,"y":543,"exts":{},"type":"ground-1"},{"x":53,"y":508,"exts":{},"type":"mage"}]'),
    bgs: JSON.parse('[{"depth":-4,"texture":"background-1","extends":{"repeat":"no-repeat","shiftY":-608,"scaleX":0.86,"scaleY":1.47,"shiftX":0}}]'),
    tiles: JSON.parse('[{"depth":-10,"tiles":[],"extends":{}}]'),
    backgroundColor: '#000000',
    ...
    this.gameStartTimer += 1;
    if (this.gameStartTimer >= 200) this.gameStartBreak = false;
    if (this.gameStartTimer%20 === 0 && !this.gameEnd) this.score += 1;
    if (this.score > 1000) {
        this.difficulty = 'hard';
    } else if (this.score > 500) {
        this.difficulty = 'medium';
    ...
    if (!this.gameStartBreak) {
        if (!this.obstacleIncoming) {
            this.obstacleIncoming = true;
            ct.types.copy('c1', ct.camera.width + 200, ct.camera.height - 130);
        }
    }
} else {
    if (ct.actions.jump.pressed) {
...
(function resAddon(ct) {
    ...
    ct.res = {
        ...
        soundsError: 0,
        sounds: {},
        registry: [{"golemSpriteSheet":{"frames":90,"shape":{"type":"rect","top":-23,"bottom":71,"left":-25,"right":74},"anchor":{"x":0,"y":0}},"mageSpriteSheet":{"frames":8,"shape":{"type":"rect","top":21,"bottom":42,"left":35,"right":12},"anchor":{"x":0.5045871559633027,"y":0.6238532110091743}},"Layer_0007_Lights":{"frames":1,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"background":{"atlas":"./img/t0.png","frames":0,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"background-1":{"atlas":"./img/t1.png","frames":0,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"trees-4":{"frames":1,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"trees-1":{"frames":1,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"trees-2":{"frames":1,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"Layer_0004_Lights":{"frames":1,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"Layer_0002_7":{"frames":1,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"trees-3":{"frames":1,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"ground-1":{"atlas":"./img/t2.png","frames":0,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0.00032126793745984356,"y":0.9272594832110156}},"ground-2":{"frames":1,"shape":{"type":"rect","top":0,"bottom":793,"left":0,"right":928},"anchor":{"x":0,"y":0}},"mage_attack":{"frames":1,"shape":{"type":"rect","top":0,"bottom":128,"left":0,"right":896},"anchor":{"x":0,"y":0}},"mage_run":{"frames":8,"shape":{"type":"rect","top":21,"bottom":42,"left":35,"right":12},"anchor":{"x":0.46218487394957986,"y":0.6238532110091743}},"mage_jump":{"frames":7,"shape":{"type":"rect","top":21,"bottom":42,"left":35,"right":21},"anchor":{"x":0.4700854700854701,"y":0.6238532110091743}},"invertedGolemSpriteSheet":{"frames":90,"shape":{"type":"rect","top":-23,"bottom":71,"left":-25,"right":74},"anchor":{"x":0,"y":0}},"invertedGolemAttack":{"frames":8,"shape":{"type":"rect","top":24,"bottom":24,"left":22,"right":27},"anchor":{"x":0.48,"y":0.37}},"mage_hurt":{"frames":4,"shape":{"type":"rect","top":21,"bottom":42,"left":35,"right":12},"anchor":{"x":0.46218487394957986,"y":0.6238532110091743}},"mage_death":{"frames":11,"shape":{"type":"rect","top":21,"bottom":20,"left":35,"right":20},"anchor":{"x":0.5544354838709677,"y":0.6626014634146342}},"mage_idle":{"frames":14,"shape":{"type":"rect","top":21,"bottom":42,"left":35,"right":18},"anchor":{"x":0.55,"y":0.6238532110091743}},"fireballSpriteSheet":{"frames":4,"shape":{"type":"rect","top":5,"bottom":6,"left":17,"right":18},"anchor":{"x":0.4772727272727273,"y":0.6904761904761905}},"invertedFireballSpritesheet":{"frames":4,"shape":{"type":"rect","top":4,"bottom":5,"left":22,"right":24},"anchor":{"x":0.29545454545454547,"y":0.6666666666666666}}}][0],
        atlases: [["./img/a0.json","./img/a1.json","./img/a2.json"]][0],
        skelRegistry: [{}][0],
...
ct.sprite(
    'golemSpriteSheet', 'Golem_Idle',
    [1, 2, 3, 4]
);
ct.sprite(
    'golemSpriteSheet', 'Golem_Glowing',
    [11, 12, 13, 14, 15, 16, 17, 18]
);
ct.sprite(
    'golemSpriteSheet', 'Golem_Ranged_Attack',
    [21, 22, 23, 24, 25, 26, 27, 28, 29]
);
ct.sprite(
    'golemSpriteSheet', 'Golem_Immune',
    [31, 32, 33, 34, 35, 36, 37, 38]
);
ct.sprite(
    'golemSpriteSheet', 'Golem_Melee_Attack',
    [41, 42, 43, 44, 45, 46, 47, 46, 45, 44, 43, 42, 41]
);
ct.sprite(
    'golemSpriteSheet', 'Golem_Test',
    [11, 12, 13, 14, 15, 16, 17, 18, 19]
);

ct.sprite(
    'mageSpriteSheet', 'Mage_Test',
    [1, 2, 3, 2, 1]
);
...
ct.sprite(
    'invertedGolemSpriteSheet', 'Inverted_Golem_Glowing',
    [19, 18, 17, 16, 15, 14, 13, 12]
);

ct.sprite(
    'invertedGolemSpriteSheet', 'Inverted_Golem_Idle',
    [10, 9, 8, 7]
);

ct.sprite(
    'invertedFireballSpritesheet', 'Inverted_Fireball',
    [3, 2, 1, 0]
);
...
ct.types.templates["golem_idle"] = {
    depth: 0,
    texture: "golemSpriteSheet",
    onStep: function () {
        this.play();
    },
    onDraw: function () {
...
ct.types.list['golem_idle'] = [];
ct.types.templates["mage"] = {
    depth: 0,
    texture: "mageSpriteSheet",
    onStep: function () {
        if (this.tex === "Mage_Hurt") {
    let timer = ct.timer.add(500, 'hurt');
    timer.then(() => {
        this.tex = "Mage_Run";
        console.log("Hurt");
    });
...
if (ct.place.occupied(this, this.x, this.y + 1, 'ground')) {
    if (ct.actions.jump.pressed && ct.delta) {
            if (ct.room.gameStart) {
            this.vspeed = this.jumpSpeed;
            this.tex = 'Mage_Jump';
            let timer = ct.timer.add(1000, 'jump');
            timer.then(() => {
                // Do something useful
                this.tex = 'Mage_Run';
                console.log('Jumped!');
...
for (var i = 0; i < Math.abs(this.vspeed); i++) {
    if (ct.delta && ct.place.free(this, this.x, this.y + Math.sign(this.vspeed), 'Solid')) {
...
ct.types.list['ground-1'] = [];
ct.types.templates["c1"] = {
    depth: 0,
    texture: "golemSpriteSheet",
    onStep: function () {
        this.play();
this.move();
let collided = ct.place.meet(this, this.x, this.y, 'mage');
if (collided) {
    if (ct.room.lives > 1) { 
        ct.room.lives -= 1;
        collided.tex = "Mage_Hurt";
    } else {
        ct.room.lives = 0;
        ct.room.gameEnd = true;
        ct.room.iterations++;
        collided.tex = "Mage_Death";
        let timer = ct.timer.add(1000, 'death');
        timer.then(() => {
            collided.kill = true;
        });
    }
    this.kill = true;
    ct.room.obstacleIncoming = false;
}
if (this.x < -25) {
    this.kill = true;
    ct.room.obstacleIncoming = false;
    ...
    onCreate: function () {
        this.tex = "Inverted_Golem_Glowing";
this.animationSpeed = 8/60;
this.scale.y = ct.room.obstacleScale;
this.detSpeed = function (room) {
    if (room.difficulty === 'easy') {
        return ct.random.range(-5, -7);
    } else if (room.difficulty === 'medium') {
        return ct.random.range(-5, -10);
    } else if (room.difficulty === 'hard') {
        return ct.random.range(-5, -12);
    } else {
        return ct.random.range(-7, -15)
    }
}
this.hspeed = this.detSpeed(ct.room);
//this.type = "obstacle";
    },
    extends: {
    "ctype": "obstacle",
...
ct.types.list['c1'] = [];
ct.types.templates["fireball"] = {
    depth: 0,
    texture: "invertedFireballSpritesheet",
    onStep: function () {
        this.move();
this.play();
    },
    ...
    onCreate: function () {
        this.tex = "Inverted_Fireball";
this.animationSpeed = 4/60;
this.hspeed = -3;
    },
    extends: {}
};
ct.types.list['fireball'] = [];
